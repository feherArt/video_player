...
#include "VideoEditor.h"
                                                                                                                    ...
                                                                                                                    //PALINDROME LOOPS MUST BE IMPLEMENTED!
int main() 
{
    ...
    cv::Size m_full_hd{ 3840, 2160};
    auto m_fourcc{ cv::VideoWriter::fourcc('H', '2', '6', '4') };
    double m_video_witer_frame_rate{ 30.0f };
    const std::string& m_output_file_name{ "erzet_II_2_comp_0_1.mp4" };
   ...
   /*for (size_t i{}; i < 30000; ++i)
    ...
    VideoEditor video_editor{ number_of_videos, ".mp4" };
    ...
    */
    ...
    std::vector<std::vector<cv::Mat>> two_dimensional_vector{ video_editor.total_random_editing_variable_lenght(300, 10)};
    for (const auto& vector : two_dimensional_vector)
        for (const auto& frame : vector)
            m_video_writer.write(frame);
